...
begin
    ...
    # 1h model datasets
    X_1h_train = master_df[1:end-168, 3:end]
    y_1h_train = master_df[1:end-168, 2]
    X_1h_test = master_df[end-167:end-167, 3:end]
    y_1h_test = master_df[end-167:end-167, 2]

    # 24h model datasets
    X_24h_train = master_df[1:end-168, [3:end-3; end-1:end]]
    y_24h_train = master_df[1:end-168, 2]
    X_24h_test = master_df[end-167:end-144, [3:end-3; end-1:end]]
    y_24h_test = master_df[end-167:end-144, 2]

    # 24h model datasets
    X_168h_train = master_df[1:end-168, [3:end-3; end]]
    y_168h_train = master_df[1:end-168, 2]
...
begin
    ...
    println("Maximum absolute error of first 24h period using 1h + 24h models: $maxAE_1h_24h L/s")
    maxAE_24h = maximum(abs.(y_24h_test .- y_24h_predict))
    println("Maximum absolute error of first 24h period using 24h model: $maxAE_24h L/s")
    maxAE_168h = maximum(abs.(y_168h_test[1:24] .- y_168h_predict[1:24]))
    println("Maximum absolute error of first 24h period using 168h model: $maxAE_168h L/s")
